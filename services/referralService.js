importimport { getGuildConfig, saveGuildConfig } from '../utils/guildConfig.js';
...
// =====================================================
// FRAUD DETECTION
// NOTE: serverId is treated as the actor in this system
// =====================================================
function detectFraud(guildId, serverId, code, ref) {
  const now = Date.now();

  // 1. SELF REFERRAL
  if (ref.ownerId === serverId) {
    return { fraud: true, reason: 'SELF_REFERRAL' };
  }

  // 2. RATE LIMIT (3 attempts / 10 min)
  const actions = recentActions.get(serverId) || [];
  ...
  filtered.push(now);
  recentActions.set(serverId, filtered);
...
export function getReferralLeaderboard(guildId, limit = 10) {
  ...
  return Object.entries(config.referrals.leaderboard)
    .sort((a, b) => b[1] - a[1])
    .slice(0, limit);
} { getGuildConfig, saveGuildConfig } from './guildConfig.js';
...
// =====================================================
// FRAUD DETECTION (ANTI-ALT / ANTI-FARM)
// =====================================================
function detectFraud(guildId, userId, code, ref) {
  ...
  // 2. RATE LIMIT PER USER (3 attempts / 10 min)
  ...
  // 4. SERVER FARM DETECTION (soft limit)
...
export function redeemReferralCode(guildId, code, serverId) {
  ...
  // 2. SERVER ALREADY USED THIS CODE
  if (config.referrals.usedServers[serverId]) {
    return { ok: false, reason: 'SERVER_ALREADY_USED_CODE' };
  }
